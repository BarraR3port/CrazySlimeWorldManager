...
public class SWMPlugin extends JavaPlugin implements SlimePlugin {
    ...
    private SlimeNMS getNMSBridge() throws InvalidVersionException {
        ...
        switch ( nmsVersion ) {
            case "v1_12_R1" :
                return new v112SlimeNMS( isPaperMC );
            case "v1_16_R3" : return new v116SlimeNMS( isPaperMC , this );
            case "v1_17_R1": return new v1171SlimeNMS( isPaperMC );
            case "v1_18_R1" :return new v1181SlimeNMS( isPaperMC );
    ...
    public void generateWorld(SlimeWorld slimeWorld) {
        ...
        PreGenerateWorldEvent preEvent = new PreGenerateWorldEvent(slimeWorld);
        ...
        PostGenerateWorldEvent postEvent = new PostGenerateWorldEvent(slimeWorld);
    ...
    public CompletableFuture<Optional<SlimeWorld>> asyncLoadWorld(SlimeLoader slimeLoader, String worldName, boolean readOnly, SlimePropertyMap slimePropertyMap) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                AsyncPreLoadWorldEvent preEvent = new AsyncPreLoadWorldEvent(slimeLoader, worldName, readOnly, slimePropertyMap);
                Bukkit.getPluginManager().callEvent(preEvent);
                ...
                SlimeWorld world = loadWorld(preEvent.getSlimeLoader(), preEvent.getWorldName(), preEvent.isReadOnly(), preEvent.getSlimePropertyMap());
                AsyncPostLoadWorldEvent postEvent = new AsyncPostLoadWorldEvent(world);
                Bukkit.getPluginManager().callEvent(postEvent);
    ...
    public CompletableFuture<Optional<SlimeWorld>> asyncGetWorld(SlimeLoader slimeLoader, String worldName) {
        return CompletableFuture.supplyAsync(() -> {
            AsyncPreGetWorldEvent preEvent = new AsyncPreGetWorldEvent(slimeLoader, worldName);
            Bukkit.getPluginManager().callEvent(preEvent);
            ...
            SlimeWorld world = getWorld(preEvent.getSlimeLoader(), preEvent.getWorldName());
            AsyncPostGetWorldEvent postEvent = new AsyncPostGetWorldEvent(world);
            Bukkit.getPluginManager().callEvent(postEvent);
            return Optional.ofNullable(postEvent.getSlimeWorld());
        });
    }
    ...
    public CompletableFuture<Optional<SlimeWorld>> asyncCreateEmptyWorld(SlimeLoader slimeLoader, String worldName, boolean readOnly, SlimePropertyMap slimePropertyMap) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                AsyncPreCreateEmptyWorldEvent preEvent = new AsyncPreCreateEmptyWorldEvent(slimeLoader, worldName, readOnly, slimePropertyMap);
                Bukkit.getPluginManager().callEvent(preEvent);
                ...
                SlimeWorld world = createEmptyWorld(preEvent.getSlimeLoader(), preEvent.getWorldName(), preEvent.isReadOnly(), preEvent.getSlimePropertyMap());
                AsyncPostCreateEmptyWorldEvent postEvent = new AsyncPostCreateEmptyWorldEvent(world);
                Bukkit.getPluginManager().callEvent(postEvent);
                return Optional.ofNullable(postEvent.getSlimeWorld());
    ...
    public CompletableFuture<Void> asyncMigrateWorld(String worldName, SlimeLoader currentLoader, SlimeLoader newLoader) {
        return CompletableFuture.runAsync(() -> {
            try {
                AsyncPreMigrateWorldEvent preEvent = new AsyncPreMigrateWorldEvent(worldName, currentLoader, newLoader);
                Bukkit.getPluginManager().callEvent(preEvent);
                ...
                AsyncPostMigrateWorldEvent postEvent = new AsyncPostMigrateWorldEvent(preEvent.getWorldName(), preEvent.getCurrentLoader(), preEvent.getNewLoader());
                Bukkit.getPluginManager().callEvent(postEvent);
    ...
    public CompletableFuture<Void> asyncImportWorld(File worldDir, String worldName, SlimeLoader slimeLoader) {
        return CompletableFuture.runAsync(() -> {
            try {
                AsyncPreImportWorldEvent preEvent = new AsyncPreImportWorldEvent(worldDir, worldName, slimeLoader);
                Bukkit.getPluginManager().callEvent(preEvent);
                ...
                AsyncPostImportWorldEvent postEvent = new AsyncPostImportWorldEvent(preEvent.getWorldDir(), preEvent.getWorldName(), preEvent.getSlimeLoader());
                Bukkit.getPluginManager().callEvent(postEvent);
